...
class TFLChangeSetProtocolSpecs : QuickSpec {
    ...
    override func spec() {
        ...
        it("should return the correct tuple when nothing's been inserted : [] -> []") {
            let newList : [M] = []
            let (inserted ,deleted ,updated, moved)  = changeSetHandler.evaluateLists(oldList: [], newList: newList, sortedBy : M.compare)
        ...
        it("should return the correct tuple when nothing's changed : [1,2,4,6,8] -> [1,2,4,6,8]") {
            ...
            let newList = [1,2,4,6,8].map { M("\($0)",$0) }
            let (inserted ,deleted ,updated, moved)  = changeSetHandler.evaluateLists(oldList: oldList, newList: newList, sortedBy : M.compare)
            expect(Set(inserted.map { Pos($0) })) == Set([])
            expect(Set(deleted.map { Pos($0) })) == Set([])
        ...
        it("should return the correct tuple when everything's removed : [1,2,4,6,8] -> []") {
            let oldList = [1,2,4,6,8].map { M("\($0)",$0) }
            let (inserted ,deleted ,updated, moved)  = changeSetHandler.evaluateLists(oldList: oldList, newList: [], sortedBy : M.compare)
            expect(Set(inserted.map { Pos($0) })) == Set([])
        ...
        xit("should return the correct tuple when moved 5 to the end plus 2 new inserts: ([5] -> [1,2,5]") {
            let oldList = [5].map { M("\($0)",$0) }
            let newList =  [M("1",1),M("2",2),M("5",5)]
            let (inserted ,deleted ,updated, moved)  = changeSetHandler.evaluateLists(oldList: oldList, newList: newList, sortedBy : M.compare)
            expect(Set(inserted.map { Pos($0) })) == Set([Pos(M("1",1),0),Pos(M("2",2),1)])
            expect(Set(deleted.map { Pos($0) })) == Set([])
            expect(Set(updated.map { Pos($0) })) == Set([])
            expect(Set(moved.map { MovedPos($0) })) == Set([MovedPos(M("5",5),0,2)])
        }
